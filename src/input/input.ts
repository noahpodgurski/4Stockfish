...
export const nullInput = () => inputData ();

export const nullInputs = () => [ inputData ( )
                                , inputData ( )
                                , inputData ( )
                                , inputData ( )
                                , inputData ( )
...
export const aiPlayer1 = [ inputData ( )
                         , inputData ( )
                         , inputData ( )
                         , inputData ( )
                         , inputData ( )
...
export const aiPlayer2 =  [ inputData ( )
                          , inputData ( )
                          , inputData ( )
                          , inputData ( )
                          , inputData ( )
...
export const aiPlayer3 =  [ inputData ( )
                          , inputData ( )
                          , inputData ( )
                          , inputData ( )
                          , inputData ( )
...
export const aiPlayer4 = [ inputData ( )
                         , inputData ( )
                         , inputData ( )
                         , inputData ( )
                         , inputData ( )
...
function pollKeyboardInputs(gameMode : number, frameByFrame :boolean, keys : {[key: number] :boolean}) : Input {
...
function pollGamepadInputs( gameMode : number, gamepadInfo : GamepadInfo
  ...
  // -------------------------------------------------------
  // analog sticks
  const fixedGamepad: Gamepad = gamepad as unknown as Gamepad; //todo fix??

  const lsVec = stickValue(fixedGamepad, gamepadInfo, "ls");
  const csVec = stickValue(fixedGamepad, gamepadInfo, "cs");
  const isGC = gamepadInfo.isGC;

  let lsCardinals: StickCardinals | null = null;
  if (gamepadInfo.ls !== null) {
    lsCardinals = gamepadInfo.ls.cardinals;
  }
  let csCardinals: StickCardinals | null = null;
  ...
  input.s = buttonState(fixedGamepad, gamepadInfo, "s");
  input.x = buttonState(fixedGamepad, gamepadInfo, "x");
  input.a = buttonState(fixedGamepad, gamepadInfo, "a");
  input.b = buttonState(fixedGamepad, gamepadInfo, "b");
  input.y = buttonState(fixedGamepad, gamepadInfo, "y");
  input.z = buttonState(fixedGamepad, gamepadInfo, "z");

  // -------------------------------------------------------
  // triggers

  input.l = buttonState(fixedGamepad, gamepadInfo, "l");
  input.r = buttonState(fixedGamepad, gamepadInfo, "r");

  if (gamepadInfo.lA !== null) {
    const lA = gamepadInfo.lA;
    if ( lA.kind === "light") {
      input.lA = triggerValue(fixedGamepad, gamepadInfo, "lA");
    }
    else {
      input.lA = scaleToGCTrigger( triggerValue(fixedGamepad, gamepadInfo, "lA")   // raw trigger value
  ...
  if (gamepadInfo.rA !== null) {
    ...
    if ( rA.kind === "light") {
      input.rA = triggerValue(fixedGamepad, gamepadInfo, "rA");
    }
    else {
      input.rA = scaleToGCTrigger( triggerValue(fixedGamepad, gamepadInfo, "rA")   // raw trigger value
  ...
  const dPadData = dPadState(fixedGamepad, gamepadInfo);
...
export function showButton(i : number, but : number,boolean :boolean) : void {
  if (boolean) {
    $("#" + i + "button" + but).show();
  }
  else {
