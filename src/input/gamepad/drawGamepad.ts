...
// controller colours
const controllerColours: ControllerColour[] = ["purple", "orange", "black", "white", "red", "blue", "green"];
...
function cycleColour( colour : ControllerColour, forward :boolean) : ControllerColour {
  const lg = controllerColours.length;
  let newColour: ControllerColour = "purple";
  for (let i = 0; i < lg; i++) {
...
export function cycleGamepadColour( i : number, which : string, forward :boolean) : void {
...
function stickSquash (pos : Vec2D, center : Vec2D) : { scalingMatrix : [[number, number], [number, number]], newCenter : Vec2D } {
  ...
  if (r < 0.01) {
    ...
    const scalingMatrix: [[number, number], [number, number]] = [[1+(f-1)*x*x/r*r, (f-1)*x*y/r*r], [(f-1)*x*y/r*r, (1+(f-1)*y*y/r*r)]];
