...
function setCalibrationInProgress(i: number,boolean:boolean): void {
...
const nullSnapshots: Snapshots = {
  b0: [], bL: [], bR: [], bU: []
  , a0: [], aL: [], aR: [], aU: []
};

export type ClickObject = null | "a" | "b" | "x" | "y" | "ls" | "cs" | "s" | "r" | "l" | "z" | "dpad"
...
export const customGamepadInfoIsUsable: Array<null |boolean> = [true, null, null, null, null, null, null, null];

let listening = false;

const ids: ClickObject[] = ["a", "b", "x", "y", "s", "r", "l", "z", "dpad", "icon", "ls", "cs"];
...
function listen(): void {
  ...
  const svgDoc = document.getElementById("gamepadSVGCalibration")?.ownerDocument;
...
export function setCustomGamepadInfoIsUsable(j: number): void {
  const currentGamepad = getGamepad(j);
  if (currentGamepad) {
...
function resetGamepadInfo(j: number): GamepadInfo {
  ...
  let baseGamepadInfo: GamepadInfo = nullGamepadInfo;
  ...
  if (clickObject === "s") {
    ...
    setTimeout(() => {
     ...
     if(gamepad !== undefined && gamepad !== null) {
       ...
       updateControllerMenu(false, errorText, 0);
     }
    }, interval);
  }
...
function scanForButton(buttons0: Array<Button>, buttons1: Array<Button>
    ...
    , onlyPressed:boolean = false): null | ButtonInfo {

  let buttonInfo: ButtonInfo = null;
...
function detectedButtonPressed(pressed0:boolean, pressed1:boolean):boolean {
  return pressed1 && !pressed0;
};

function detectedButtonValue(value0: number, value1: number):boolean {
  return (value0 < 0.25 && value1 > 0.75);
};


function scanForTrigger(buttons0: Array<Button>, buttons1: Array<Button>
    , axes0: Array<number>, axes1: Array<number>): null | TriggerInfo {
  let minMax;
  let triggerInfo: TriggerInfo = null;
...
function scanForStick(buttons0: Array<Button>
  ...
  if (xIndex !== undefined && yIndex !== undefined && Math.abs(xDiff) > 0.5 && Math.abs(yDiff) > 0.5) {
    cardinals = {
      center: new Vec2D(axes0[xIndex], axes0[yIndex])
      , left: axesL[xIndex]
      , right: axesR[xIndex]
      , up: axesU[yIndex]
      , down: axesD[yIndex]
  ...
  if (Math.abs(xDiff) < 0.5 || Math.abs(yDiff) < 0.5) {
    ...
    if (xIndex !== undefined && yIndex !== undefined && Math.abs(xDiff) > 0.5 && Math.abs(yDiff) > 0.5) {
      cardinals = {
        center: new Vec2D(buttons0[xIndex].value, buttons0[yIndex].value)
