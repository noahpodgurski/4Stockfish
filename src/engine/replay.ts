let player, playing, playerType, startGame, characterSelections, setCS, addPlayer, stageSelect, setStageSelect, endGame, finishGame, ports, starting, pause, gameEnd;
(async () => {
  ;
  if (process.env.RUN_MODE === 'engine') {
    const engineModule = await import('../engine/main');
    ({ player, playing, playerType, startGame, characterSelections, setCS, addPlayer, stageSelect, setStageSelect, endGame, finishGame, ports, starting, pause, gameEnd } = engineModule);
  } else {
    const mainModule = await import('../main/main');
    ({ player, playing, playerType, startGame, characterSelections, setCS, addPlayer, stageSelect, setStageSelect, endGame, finishGame, ports, starting, pause, gameEnd } = mainModule);
  }
})();
...
export function loadReplay(file) {
  ...
  reader.onload = function (event) {
    ...
    for (let n = 0; n < replayInputPackage.length; n++) {
      const stateData: any = decompressObject(replayInputPackage[n]);
...
export function retrieveReplayInputs(playerSlot, controllerIndex) {
