...
export default (posInQueue)=> {
  ...
  switch (vfxQueue[posInQueue].timer) {
    case 1:
      ...
      break;
    case 2:
      drawArrayPath(fg2, "rgb(50,252,162)", vfxQueue[posInQueue].face, 0, 0, vfx.normalhit.path1, 0.2 * (activeStage.scale / 4.5),
          0.2 * (activeStage.scale / 4.5));
      break;
    case 3:
      drawArrayPath(fg2, "rgb(0,0,0)", vfxQueue[posInQueue].face, 0, 0, vfx.normalhit.path2, 0.2 * (activeStage.scale / 4.5), 0.2 *
          (activeStage.scale / 4.5));
      break;
    case 4:
      drawArrayPath(fg2, "rgb(198,222,255)", vfxQueue[posInQueue].face, 0, 0, vfx.normalhit.path2, 0.2 * (activeStage.scale / 4.5),
          0.2 * (activeStage.scale / 4.5));
      break;
    case 5:
      for (let n = 0; n < vfx.normalhit.path3.length; n++) {
        drawArrayPath(fg2, "rgb(0,0,0)", vfxQueue[posInQueue].face, 0, 0, vfx.normalhit.path3[n], 0.2 * (vfxQueue[posInQueue].timer / 7) *
            (activeStage.scale / 4.5), 0.2 * (vfxQueue[posInQueue].timer / 7) * (activeStage.scale / 4.5));
      }
      break;
    case 6:
      for (let m = 0; m < vfx.normalhit.path3.length; m++) {
        drawArrayPath(fg2, "rgb(139,130,242)", vfxQueue[posInQueue].face, 0, 0, vfx.normalhit.path3[m], 0.2 * (vfxQueue[posInQueue].timer /
            7) * (activeStage.scale / 4.5), 0.2 * (vfxQueue[posInQueue].timer / 7) * (activeStage.scale / 4.5));
      }
      break;
    default:
      break;
  ...
  if (vfxQueue[posInQueue].timer < 13) {
    for (let i = 0; i < 2; i++) {
      fg2.beginPath();
  ...
  for (let j = 0;  (vfxQueue[posInQueue]) && j < 4 - (Math.round(vfxQueue[posInQueue].timer / 4)); j++) {
