...
export function drawSSSInit() {
  ...
  for (let i = 0; i < smallBoxStageNames.length; i++) {
    fg1.fillRect(87.5 + i * xRowOffset, 450, 150, 90);
  }
  ...
  for (let i = 0 ; i < smallBoxStageNames.length ; i++) {
    fg1.fillText(smallBoxStageNames[i], i * xRowOffset + 162.5, 530);
    fg1.drawImage(stageImages[i ], i * xRowOffset + 87.5, 452, 146, 55);
  }
...
export function drawSSS() {
  ...
  for (var i = 0; i < smallBoxStageNames.length; i++) {
    ...
    ui.strokeRect(87.5 + i * xRowOffset, 450, 150, 90);
  }
