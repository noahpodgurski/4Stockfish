...
export function updateControllerMenu(quit, texts, interval){
  ...
  if (quit) {
    setTimeout(function(){ 
      const gpd: HTMLElement | null = document.getElementById("gamepadSVGCalibration");
      if (gpd === null) {
        throw Error("Unable to find gamepadSVGCalibration")
      }
      gpd.style.display = "none";
      const canvas: HTMLCanvasElement | null = document.getElementById('uiCanvas') as HTMLCanvasElement | null;
      if (canvas === null) {
...
export function drawControllerMenuInit (){
  ...
  const cal: HTMLElement | null = document.getElementById("gamepadSVGCalibration")
  if (cal !== null) {
    cal.style.display = "";
  }
...
let customInteract: number | null = null;
let customInUse = 0;
let saveOrLoad = "load";
...
function fillColour (state, k?: number) {
...
function strokeColour (state, k?: number) {
  if (state === "pressed") {
...
function clickFunction (e) {
  ...
  if (x >= 30 && x <= 150) {
    if (y >= 60 && y <= 120) {
      ...
      setClickObject(saveOrLoad+"Custom" as ClickObject);
    }
    else if (y >= 240 && y <= 300) {
