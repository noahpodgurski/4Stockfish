    ...
    if (handPos[i].y > 640 && handPos[i].y < 680 && handPos[i].x > 130 + i * 225 && handPos[i].x < 310 + i * 225) {
      ...
      if (input[i][0].a && !input[i][1].a) {
        ...
        if (handPos[i].x < 154 + i * 225) {
          ...
          ui.fillRect(0, 0, (layers?.UI?.width ?? 0), (layers?.UI?.height ?? 0));
...
export function drawCSSInit() {
  ...
  bg1.save();
  bg1.lineWidth = 3;
  bg1.translate((layers?.BG1?.width ?? 0) / 2, (layers?.BG1?.height ?? 0) / 2 + 20);
  for (var i = 0; i < 2; i++) {
    bg1.rotate(i * Math.PI);
    bg1.beginPath();
    bg1.moveTo(-10 - (layers?.BG1?.width ?? 0) / 2, -250);
    bg1.lineTo(-300, -250);
    bg1.bezierCurveTo(-240, -250, -240, -330, -180, -330);
    bg1.lineTo(10 + (layers?.BG1?.width ?? 0) / 2, -330);
    bg1.lineTo(10 + (layers?.BG1?.width ?? 0) / 2, -30 - (layers?.BG1?.height ?? 0) / 2);
    bg1.lineTo(-10 - (layers?.BG1?.width ?? 0) / 2, -30 - (layers?.BG1?.height ?? 0) / 2);
    bg1.closePath();
    bg1.fill();
    bg1.stroke();
...
export function drawCSS() {
  ...
  for (let i = 3; i >= 0; i--) {
    if (playerType[i] > -1) {
      ...
      ui.fillStyle = "rgba(0,0,0,0.4)";
      ui.beginPath();
      ui.moveTo(tokenPos[i].x, tokenPos[i].y); //changed from ui.beginPath(tokenPos[i].y);
