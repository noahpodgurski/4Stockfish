...
import { surfaceToVec2DArray, Vec2DArrayToSurface } from "../stages/encode";
...
function runPointSweep(ecb1: ECB, ecbp: ECB, same: number
  ...
  let result: PointSweepResult | null = null;
  ...
  if (isOutside(ecb1[pt], wallTopOrRight, wallBottomOrLeft, wallType) && !isOutside(ecbp[pt], wallTopOrRight, wallBottomOrLeft, wallType)) {
    ...
    if (!(isNaN(s) || s === Infinity || s > 1 || s < 0)) {
      ...
      if (getXOrYCoord(intersection, xOrY) <= getXOrYCoord(wallTopOrRight, xOrY) && getXOrYCoord(intersection, xOrY) >= getXOrYCoord(wallBottomOrLeft, xOrY)) {
        result = {sweep: s, kind: "surface", surface: Vec2DArrayToSurface(wall), type: wallType, index: wallIndex, pt: pt};
      }
    }
  }
...
export type EdgeSweepResult = { kind : "corner", corner : Vec2D, sweep : number, angular : number, damageType? : DamageType}
...
function runEdgeSweep(ecb1: ECB, ecbp: ECB, same: number
  ...
  let corner: Vec2D | null = null;
  let otherCorner: Vec2D | null = null;

  let edgeSweepResult: EdgeSweepResult | null = null;
  let otherEdgeSweepResult: EdgeSweepResult | null = null;
  ...
  return pickSmallestSweep([edgeSweepResult, otherEdgeSweepResult]) as EdgeSweepResult;
...
function edgeSweepingCheck(ecb1: ECB, ecbp: ECB, same: number, other: number
  ...
  let output: EdgeSweepResult | null = null;
  ...
  if (!isOutside(corner, ecbp[same], ecbp[other], interiorECBside) && isOutside(corner, ecb1[same], ecb1[other], interiorECBside)) {
    ...
    const recenteredECB1Edge: [Vec2D, Vec2D] = [new Vec2D(ecb1[same].x - corner.x, ecb1[same].y - corner.y)
      , new Vec2D(ecb1[other].x - corner.x, ecb1[other].y - corner.y)];
    const recenteredECBpEdge: [Vec2D, Vec2D] = [new Vec2D(ecbp[same].x - corner.x, ecbp[same].y - corner.y)
...
export type CollisionDatum = null | PointSweepResult | EdgeSweepResult
...
type PlayerStatusInfo = { grounded :boolean, ignoringPlatforms :boolean, immune :boolean };

type ECBDatum = { ecb : ECB, touching : null | SimpleTouchingDatum, squashed :boolean };
...
function runSlideRoutine(srcECB: ECB, tgtECB: ECB, ecbp: ECB
  ...
  if (recursionCounter > maxRecursion) {
    ...
    if (slideDatum.event === "end") {
      ...
      let newFinal;
      let newTgtECB;
      let newSlidingType: "l" | "r" | "c" | null = null;
      let same;
      let other;
      ...
      if (slideObject.kind === "surface") {
        ...
        [newTgtECB, newFinal] = findNextTargetFromSurface(newSrcECB, newECBp, surfaceToVec2DArray(surface), surfaceType, angular);
      }
      else {
...
function slideECB(srcECB: ECB, tgtECB: ECB
    , labelledSurfaces: Array<LabelledSurface>
    , slidingAgainst: Sliding
    , playerStatusInfo: PlayerStatusInfo): SlideDatum {
  let output;
...
function findNextTargetFromSurface(srcECB: ECB, ecbp: ECB, wall: [Vec2D, Vec2D], wallType: string, pt: number): [ECB,boolean] {
  let wallForward;
  let s = 1;
  let tgtECB = ecbp;
  let pushout = 0;
...
function findNextTargetFromCorner(srcECB: ECB, ecbp: ECB, corner: Vec2D, angularParameter: number): [ECB,boolean] {
...
function updateECBp(startECB: ECB, endECB: ECB, ecbp: ECB, slidingType: null | string, pt: number): ECB {
  if (slidingType === null) {
    ...
    let xOrY: XOrY = (slidingType === "l" || slidingType === "r") ? "y" : "x";
    let t;
...
function agreeOnTargetECB(srcECB: ECB, fstTgtECB: ECB, sndTgtECB: ECB, ecbp: ECB, pt: number, grounded:boolean): [ECB,boolean] {
  let output;
  ...
  const tgtECB: ECB = [new Vec2D(0, 0), new Vec2D(0, 0), new Vec2D(0, 0), new Vec2D(0, 0)]; // initialising
  let abort;
  let squashFactor = 1;
  ...
  if (Math.abs(otherTgtECB[same].x - closestTgtECB[diff].x) > smallestECBWidth
    ...
    const sameLine: [Vec2D, Vec2D] = [srcECB[same], otherTgtECB[same]];
    const diffLine: [Vec2D, Vec2D] = [srcECB[diff], closestTgtECB[diff]];
    const offsetDiffLine: [Vec2D, Vec2D] = [add(diffLine[0], new Vec2D(sign * smallestECBWidth, 0))
...
export function moveAlongGround(pos: Vec2D, posNext: Vec2D, ecbHeight: number, ground: Surface, ceilings: Array<Surface>): null | number {
  if (pos.x === posNext.x) {
    ...
    if ((dir === "l" && pos.x < groundLeft.x)
      ...
      const groundStart = coordinateIntercept(surfaceToVec2DArray(ground), vLineAt(start));
      const groundEnd = coordinateIntercept(surfaceToVec2DArray(ground), vLineAt(end));
      ...
      if (firstCeilingCollision === null) {
        if (ecbHeight > smallestECBHeight) {
          ...
          if (firstCeilingCollision === null || firstCeilingCollision.object.kind === "corner") {
            ...
            const intercept = coordinateIntercept(surfaceToVec2DArray(ceiling), [add(groundStart, new Vec2D(0, smallestECBHeight)), add(groundEnd, new Vec2D(0, smallestECBHeight))]);
            /*
...
function reinflateECB(ecb: ECB, position: Vec2D
    ...
    , grounded:boolean): [Vec2D, SquashDatum, ECB] {
  let q = 1;
...
export function runCollisionRoutine(ecb1: ECB, ecbp: ECB, position: Vec2D
    ...
    , playerStatusInfo: PlayerStatusInfo
    , stage: Stage): CollisionRoutineResult {
  ...
  const newSquashFactor = resolution.squashed ? Math.min(1, (newECBp[1].x - newECBp[3].x) / (ecbp[1].x - ecbp[3].x))
      : 1;
  let newSquashLocation: number | null = null;
  if (newTouching !== null) {
