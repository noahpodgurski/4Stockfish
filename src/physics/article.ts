...
export let aArticles: any[] = [];
export let destroyArticleQueue: any[] = [];
export let articleHitQueue: any[] = [];
...
export function wallDetection (i){
  ...
  let collisions: (PointSweepResult | EdgeSweepResult)[] = [];
  let thisCollision: CollisionDatum = null;
  for (var j = 0; j < activeStage.wallL.length; j++) {
    thisCollision = findCollision (
        ecb1, 
        ecbp, 
        {surface: activeStage.wallL[j], label: ["l", j]} as LabelledSurface);
  ...
  for (var j = 0; j < activeStage.wallR.length; j++) {
    thisCollision = findCollision (
        ecb1, 
        ecbp, 
        {surface: activeStage.wallR[j], label: ["r", j]} as LabelledSurface);
    if (thisCollision !== null ) {
      collisions.push(thisCollision);
    }
  }
  for (var j = 0; j < activeStage.ceiling.length; j++) {
    thisCollision = findCollision (
        ecb1, 
        ecbp, 
        {surface: activeStage.ceiling[j], label: ["c", j]} as LabelledSurface);
    if (thisCollision !== null ) {
      collisions.push(thisCollision);
    }
  }
  for (var j = 0; j < activeStage.ground.length; j++) {
    thisCollision = findCollision (
        ecb1, 
        ecbp, 
        {surface: activeStage.ground[j], label: ["g", j]} as LabelledSurface);
    if (thisCollision !== null ) {
      collisions.push(thisCollision);;
    }
  }
  for (var j = 0; j < activeStage.platform.length; j++) {
    thisCollision = findCollision (
        ecb1, 
        ecbp, 
        {surface: activeStage.platform[j], label: ["p", j]} as LabelledSurface);
    if (thisCollision !== null ) {
      collisions.push(thisCollision);
    }
  }
