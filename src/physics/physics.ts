...
import type {Ledge, Connected, Surface, SurfaceLabel} from "../stages/stage";
// eslint-disable-next-line no-duplicate-imports
...
function dealWithDamagingStageCollision(i: number, normal: Vec2D, corner:boolean, angular: number, damageType: DamageType): void {
...
function fallOffGround(i: number, side: string
    ...
    , disableFall:boolean, input: any): [boolean,boolean] {
...
function dealWithGround(i: number, ground: Surface, groundTypeAndIndex: [string, number], connected: Connected, input: any): [boolean,boolean] {

  const damageType: DamageType | null = ground[2] ? ground[2].damageType : null;
  ...
  let maybeLeftGroundTypeAndIndex: SurfaceLabel | null = null;
  let maybeRightGroundTypeAndIndex: SurfaceLabel | null = null;
  ...
  if ( player[i].phys.ECBp[0].x < leftmostGroundPoint.x) {
    ...
    const yIntercept = coordinateIntercept([ecbpBottom, new Vec2D(ecbpBottom.x, ecbpBottom.y + 1)], [vec1, vec2]);
...
export function land(i: number, newPosition: Vec2D
    , t: number, j: number, normal: Vec2D, input: any): void {
  player[i].phys.pos = newPosition;
...
function findAndResolveCollisions(i: number, input: any, 
  oldBackward:boolean, 
  oldNotTouchingWalls: boolean[], 
  ecbOffset: [number, number, number, number]): [boolean,boolean, boolean[]] {
  ...
  if (player[i].phys.grounded) {
    ...
    const relevantGroundIndex: number = player[i].phys.onSurface[1];
    let relevantGroundType = "g";
    let relevantGround: Surface = activeStage.ground[relevantGroundIndex];
    ...
    const relevantGroundTypeAndIndex: [string, number] = [relevantGroundType, relevantGroundIndex];
...
export function physics (i : number, input : any) : void {
  ...
  if (!actionStates[characterSelections[i]][player[i].actionState].ignoreCollision) {
    ...
    [stillGrounded, backward, notTouchingWalls] = findAndResolveCollisions(i, input, backward, notTouchingWalls, ecbOffset as [number, number, number, number]);
  ...
  if (showDebug) {
    let actState = document.getElementById('actState' + i)?.innerHTML ?? "";
    let stateNum = document.getElementById('stateNum' + i)?.innerHTML ?? "";
    let face = document.getElementById('face' + i)?.innerHTML ?? "";
    let velocityX = document.getElementById("velocityX" + i)?.innerHTML ?? "";
    let velocityY = document.getElementById("velocityY" + i)?.innerHTML ?? "";
    let kvelocityX = document.getElementById("kvelocityX" + i)?.innerHTML ?? "";
    let kvelocityY = document.getElementById("kvelocityY" + i)?.innerHTML ?? "";
    let pvelocityX = document.getElementById("pvelocityX" + i)?.innerHTML ?? "";
    let pvelocityY = document.getElementById("pvelocityY" + i)?.innerHTML ?? "";
    actState = player[i].currentAction + " " + player[i].currentSubaction + " : " + player[i].actionState;
    stateNum = frame.toString();
    face = player[i].phys.face;
    velocityX = player[i].phys.cVel.x.toFixed(5);
    velocityY = player[i].phys.cVel.y.toFixed(5);
    kvelocityX = player[i].phys.kVel.x.toFixed(5);
    kvelocityY = player[i].phys.kVel.y.toFixed(5);
