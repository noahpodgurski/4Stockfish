...
export function createStageCode ( stage : Stage ) : string {
  ...
  for (let n=0;n<types.length;n++) {
    tCode += "&";
    const surfaces: any = deepValue(stage,types[n]);
  ...
  for (let n=0; n < ptypes.length; n++) {
    tCode += "&";
    const polys: any = deepValue(stage, ptypes[n]);
    if (polys !== undefined) {
...
export function Vec2DArrayToSurface(vec2Array: [Vec2D, Vec2D]): Surface {
  return [vec2Array[0], vec2Array[1], void 0];
}
...
function parseSurface ( s1 : string, s2 : string, s3 : string, s4 : string, s5 : void | string ) : Surface {
  ...
  return [parseVec2D(s1,s2), parseVec2D(s3,s4), void 0];
}
...
function parseLedge ( s1 : string, s2 : string, s3 : string) : Ledge {
  ...
  if (s1[0] === "p") {
    return [ "platform", parseInt(s2), side as 0 | 1];
  }
  else {
    return [ "ground", parseInt(s2), side as 0 | 1];
  }
}
...
export function parseStageCode ( code : string ) : null | Stage {
  ...
  try {
    ...
    stage.startingPoint = sep(objects[0 ],'~').map((s) => parseVec2D  (...sep(s,',') as [string, string]));
    stage.startingFace  = sep(objects[1 ],',').map((s) => parseSign   (   s         ));
    stage.ground        = sep(objects[2 ],'~').map((s) => parseSurface(...sep(s,',') as [string, string, string, string, void | string]));
    stage.ceiling       = sep(objects[3 ],'~').map((s) => parseSurface(...sep(s,',') as [string, string, string, string, void | string]));
    stage.wallL         = sep(objects[4 ],'~').map((s) => parseSurface(...sep(s,',') as [string, string, string, string, void | string]));
    stage.wallR         = sep(objects[5 ],'~').map((s) => parseSurface(...sep(s,',') as [string, string, string, string, void | string]));
    stage.platform      = sep(objects[6 ],'~').map((s) => parseSurface(...sep(s,',') as [string, string, string, string, void | string]));
    stageBG.line        = sep(objects[7 ],'~').map((s) => surfaceToVec2DArray(parseSurface(...sep(s,',') as [string, string, string, string, void | string])));
    stage.polygon       = sep(objects[8 ],'~').map((s) => parsePolygon(   sep(s,',')));
    stageBG.polygon     = sep(objects[9 ],'~').map((s) => parsePolygon(   sep(s,',')));
    stage.ledge         = sep(objects[10],'~').map((s) => parseLedge  (...sep(s,',') as [string, string, string]));
    stage.target        = sep(objects[11],'~').map((s) => parseVec2D  (...sep(s,',') as [string, string]));
    stage.blastzone     = parseBox2D(...sep(objects[12],',') as [string, string, string, string]) || new Box2D([-250,-250],[250,250]);
