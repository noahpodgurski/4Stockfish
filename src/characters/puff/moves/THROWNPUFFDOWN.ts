...
const THROWNPUFFDOWN: State = {
  ...
  init: function (p, input) {
    ...
    puff.THROWNPUFFDOWN.main!(p, input);
  },
  main: function (p, input) {
    player[p].timer++;
    if (!puff.THROWNPUFFDOWN.interrupt!(p, input)) {
      ...
      if (timer > 0) {
        ...
        if(timer > puff.THROWNPUFFDOWN.offset.length){
          timer = puff.THROWNPUFFDOWN.offset.length -1;
        }
        player[p].phys.pos = new Vec2D(player[grabbedBy].phys.pos.x + puff.THROWNPUFFDOWN.offset[timer - 1][0] * player[p].phys.face, player[grabbedBy].phys.pos.y + puff.THROWNPUFFDOWN.offset[timer - 1][1]);
      }
    }
  },
