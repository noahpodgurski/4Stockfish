  ...
  main: function (p, input) {
    ...
    if (!marth.THROWNPUFFFORWARD.interrupt!(p, input)) {
      ...
      if (timer > 0) {
        ...
        if(timer > marth.THROWNPUFFFORWARD.offset.length){
          timer = marth.THROWNPUFFFORWARD.offset.length -1;
        }
        player[p].phys.pos = new Vec2D(player[grabbedBy].phys.pos.x + marth.THROWNPUFFFORWARD.offset[timer - 1][0] * player[p].phys.face, player[grabbedBy].phys.pos.y + marth.THROWNPUFFFORWARD.offset[timer - 1][1]);
      }
    }
  },
