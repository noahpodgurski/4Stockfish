...
exports.updateGamepadSVGColour = updateGamepadSVGColour;
exports.cycleGamepadColour = cycleGamepadColour;
exports.setGamepadSVGColour = setGamepadSVGColour;
exports.updateGamepadSVGState = updateGamepadSVGState;
...
function cycleColour(colour, forward) {
...
function updateGamepadSVGColour(i, id) {
...
;
function cycleGamepadColour(i, which, forward) {
...
function setGamepadSVGColour(i, col) {
    gamepadStates[i].colour = col;
}
function updateGamepadSVGState(i, id, maybeInput) {
    let input = maybeInput;
    if (input === null || input === undefined) {
        input = (0, input_1.nullInput)();
    }
    const svgObject = document.getElementById(id);
    if (svgObject === null || svgObject === undefined) {
    ...
    const dPadAxes = new Vec2D_1.Vec2D(0, 0);
    ...
    const cScale = 47;
    const dScale = 5;
    let lStickSquashVector = new Vec2D_1.Vec2D(input.rawX, input.rawY);
    const lStickSquashVectorNorm = (0, linAlg_1.norm)(lStickSquashVector);
    if (lStickSquashVectorNorm > 1) {
        lStickSquashVector = (0, linAlg_1.scalarProd)(1 / (1.5 * lStickSquashVectorNorm), lStickSquashVector);
    }
    else {
        lStickSquashVector = (0, linAlg_1.scalarProd)(1 / 1.5, lStickSquashVector);
    }
    let cStickSquashVector = new Vec2D_1.Vec2D(input.rawcsX, input.rawcsY);
    const cStickSquashVectorNorm = (0, linAlg_1.norm)(cStickSquashVector);
    if (cStickSquashVectorNorm > 1) {
        cStickSquashVector = (0, linAlg_1.scalarProd)(1 / (1.6 * cStickSquashVectorNorm), cStickSquashVector);
    }
    else {
        cStickSquashVector = (0, linAlg_1.scalarProd)(1 / 1.6, cStickSquashVector);
    }
    const lStickSquash = stickSquash(lStickSquashVector, new Vec2D_1.Vec2D(lsCenterX, lsCenterY));
    const cStickSquash = stickSquash(cStickSquashVector, new Vec2D_1.Vec2D(csCenterX, csCenterY));
    const dPadSquash = stickSquash(new Vec2D_1.Vec2D(dPadAxes.x / 2.5, dPadAxes.y / 2.5), new Vec2D_1.Vec2D(dCenterX, dCenterY));
    ...
    const cRectScale = 20 * (0, linAlg_1.norm)(new Vec2D_1.Vec2D(input.rawcsX, input.rawcsY));
...
function stickSquash(pos, center) {
    const x = pos.x;
    const y = -pos.y;
    const r = Math.sqrt(x * x + y * y);
    if (r < 0.01) {
        return { scalingMatrix: [[1, 0], [0, 1]], newCenter: new Vec2D_1.Vec2D(0, 0) };
    }
    else {
        const f = Math.max(0, 1 - r); // scaling factor
        const scalingMatrix = [[1 + (f - 1) * x * x / r * r, (f - 1) * x * y / r * r], [(f - 1) * x * y / r * r, (1 + (f - 1) * y * y / r * r)]];
        const mult = (0, linAlg_1.multMatVect)(scalingMatrix, [center.x, center.y]);
        return { scalingMatrix: scalingMatrix, newCenter: new Vec2D_1.Vec2D(center.x - mult[0], center.y - mult[1]) };
    }
}
