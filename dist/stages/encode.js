...
exports.surfaceToVec2DArray = surfaceToVec2DArray;
exports.Vec2DArrayToSurface = Vec2DArrayToSurface;
exports.parseStageCode = parseStageCode;
const Box2D_1 = require("../main/util/Box2D");
const Vec2D_1 = require("../main/util/Vec2D");
const deepValue_1 = require("../main/util/deepValue");
const getConnected_1 = require("../target/util/getConnected");
function createStageCode(stage) {
    ...
    for (let n = 0; n < types.length; n++) {
        tCode += "&";
        const surfaces = (0, deepValue_1.deepValue)(stage, types[n]);
    ...
    for (let n = 0; n < ptypes.length; n++) {
        tCode += "&";
        const polys = (0, deepValue_1.deepValue)(stage, ptypes[n]);
        if (polys !== undefined) {
...
function parseVec2D(s1, s2) {
    return new Vec2D_1.Vec2D(parseFloat(s1), parseFloat(s2));
}
function parseBox2D(s1, s2, s3, s4) {
    return new Box2D_1.Box2D([parseFloat(s1), parseFloat(s2)], [parseFloat(s3), parseFloat(s4)]);
}
function surfaceToVec2DArray(surface) {
    const [vec1, vec2] = surface;
    return [vec1, vec2];
}
function Vec2DArrayToSurface(vec2Array) {
...
function parseStageCode(code) {
    const stage = { startingPoint: [],
        ...
        target: [],
        scale: 3,
        blastzone: new Box2D_1.Box2D([-250, -250], [250, 250]),
        offset: [600, 375],
        connected: [[], []],
    ...
    try {
        ...
        stageBG.line = sep(objects[7], '~').map((s) => surfaceToVec2DArray(parseSurface(...sep(s, ','))));
        ...
        stage.blastzone = parseBox2D(...sep(objects[12], ',')) || new Box2D_1.Box2D([-250, -250], [250, 250]);
        stage.scale = parseFloat(objects[13]) || 3;
        stage.ledgePos = (stage.ledge).map((l) => stage[l[0]][l[1]][l[2]]);
        stage.connected = (0, getConnected_1.getConnected)(stage);
