...
const detectIntersections_1 = require("../stages/util/detectIntersections");
...
function sweepCircleVsSweepCircle(p1, r1, p2, r2, q1, s1, q2, s2) {
    if ((0, linAlg_1.euclideanDist)(p1, q1) < (r1 + s1)) {
        return new Vec2D_1.Vec2D(0.5 * p1.x + 0.5 * q1.x, 0.5 * p1.y + 0.5 * q1.y);
    }
    else {
        ...
        const t1 = (0, solveQuadraticEquation_1.solveQuadraticEquation)(a0, a1, a2);
        let t = null;
        if (t1 !== null && !isNaN(t1)) {
            const t2 = a0 / (a2 * t1);
            if (t1 < 0 || t1 > 1) {
        ...
        if (t === null) {
            ...
            const r = (1 - t) * r1 + t * r2;
            const s = (1 - t) * s1 + t * s2;
            const p = new Vec2D_1.Vec2D((1 - t) * p1.x + t * p2.x, (1 - t) * p1.y + t * p2.y);
            const q = new Vec2D_1.Vec2D((1 - t) * q1.x + t * q2.x, (1 - t) * q1.y + t * q2.y);
            const wp = s / (r + s);
            const wq = r / (r + s);
            return new Vec2D_1.Vec2D(wp * p.x + wq * q.x, wp * p.y + wq * q.y);
        }
    }
}
...
function sweepCircleVsAABB(p1, r1, p2, r2, bl, tr) {
    const br = new Vec2D_1.Vec2D(tr.x, bl.y);
    const tl = new Vec2D_1.Vec2D(bl.x, tr.y);
    if ((0, detectIntersections_1.distanceToPolygon)(p1, [bl, br, tr, tl]) <= r1) {
        return p1;
    }
        ...
        if (p1.x <= bl.x) {
            if (p1.y <= bl.y) { // bottom left corner
                checks = [{ case: "corner", corner: bl, p1: p1, p2: p2, r1: r1, r2: r2 },
                    ...
                    { case: "line", line1: (0, environmentalCollision_1.vLineThrough)(bl), line2: [(0, linAlg_1.add)(p1, new Vec2D_1.Vec2D(r1, 0)), (0, linAlg_1.add)(p2, new Vec2D_1.Vec2D(r2, 0))] },
                    { case: "line", line1: (0, environmentalCollision_1.hLineThrough)(bl), line2: [(0, linAlg_1.add)(p1, new Vec2D_1.Vec2D(0, r1)), (0, linAlg_1.add)(p2, new Vec2D_1.Vec2D(0, r2))] }
                ];
            }
                ...
                checks = [{ case: "corner", corner: tl, p1: p1, p2: p2, r1: r1, r2: r2 },
                    ...
                    { case: "line", line1: (0, environmentalCollision_1.vLineThrough)(bl), line2: [(0, linAlg_1.add)(p1, new Vec2D_1.Vec2D(r1, 0)), (0, linAlg_1.add)(p2, new Vec2D_1.Vec2D(r2, 0))] },
                    { case: "line", line1: (0, environmentalCollision_1.hLineThrough)(tr), line2: [(0, linAlg_1.add)(p1, new Vec2D_1.Vec2D(0, -r1)), (0, linAlg_1.add)(p2, new Vec2D_1.Vec2D(0, -r2))] }
                ];
            }
            else { // left side
                checks = [{ case: "corner", corner: bl, p1: p1, p2: p2, r1: r1, r2: r2 },
            ...
            if (p1.y <= bl.y) { // bottom right corner
                checks = [{ case: "corner", corner: br, p1: p1, p2: p2, r1: r1, r2: r2 },
                    { case: "corner", corner: bl, p1: p1, p2: p2, r1: r1, r2: r2 },
                    { case: "corner", corner: tr, p1: p1, p2: p2, r1: r1, r2: r2 },
                    { case: "line", line1: (0, environmentalCollision_1.vLineThrough)(tr), line2: [(0, linAlg_1.add)(p1, new Vec2D_1.Vec2D(-r1, 0)), (0, linAlg_1.add)(p2, new Vec2D_1.Vec2D(-r2, 0))] },
                    { case: "line", line1: (0, environmentalCollision_1.hLineThrough)(bl), line2: [(0, linAlg_1.add)(p1, new Vec2D_1.Vec2D(0, r1)), (0, linAlg_1.add)(p2, new Vec2D_1.Vec2D(0, r2))] }
                ];
            }
        ...
        const first = (0, findSmallestWithin_1.pickSmallestSweep)(checks.map(aabbChecker));
        if (first === null) {
            return null;
        }
        else {
...
function aabbChecker(check) {
    if (check.case === "corner") {
        ...
        const r1 = check.r1;
        const r2 = check.r2;
        if ((0, linAlg_1.euclideanDist)(c, p1) < r1) {
        ...
        const s = (0, environmentalCollision_1.coordinateInterceptParameter)(check.line2, check.line1);
        if (s < 0 || s > 1 || isNaN(s) || s === Infinity) {
            return null;
        }
        else {
            const pt = new Vec2D_1.Vec2D((1 - s) * p.x + s * q.x, (1 - s) * p.y + s * q.y);
